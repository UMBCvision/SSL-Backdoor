...
class AlexNet(nn.Module):
    def __init__(self, opt):
        ...
        rotnet_classes = opt['rotnet_classes']
        jgg_classes = opt['jgg_classes']
        ...
        self.rotnet_classifier = nn.Sequential(
            nn.Linear(4096, rotnet_classes),
        )
        self.jgg_classifier = nn.Sequential(
            nn.Linear(4096, jgg_classes),
        )
    ...
    def forward(self, x, out_feat_keys=None, run_rotnet=True):
        ...
        if run_rotnet:
            return self.rotnet_classifier(out_feats)
        else:
            return self.jgg_classifier(out_feats)
